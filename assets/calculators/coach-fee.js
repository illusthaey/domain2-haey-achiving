...
function calcCoachDistribution(studentCount, unitFee) {
  ...
  for (let i = 0; i < maxIter; i++) {
    ...
    // 개인부담금 합계
    sumEmp = pensionEmp + healthEmp + longtermEmp + employEmp;

    // 기관부담금 합계 (사회보험 + 산재)
    sumOrg = pensionOrg + healthOrg + longtermOrg + employOrg + accidentOrg;

    // 공제합계 = 개인 + 기관 (퇴직/연차 증가분 제외)
...
// 버튼 연결(인라인 onclick 제거용)
document.addEventListener("DOMContentLoaded", () => {
  const btn = document.getElementById("btnCoachCalc");
